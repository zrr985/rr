...
#define FLAME_LABEL_NALE_TXT_PATH "../model/flame_labels_list.txt"
#define SMOKING_LABEL_NALE_TXT_PATH "../model/smoking_labels_list.txt"
#define FACE_LABEL_NALE_TXT_PATH "../model/face_labels_list.txt"
...
static int loadFlameLabelName(const char *locationFilename, char *label[])
{
    printf("load flame lable %s\n", locationFilename);
    readLines(locationFilename, label, FLAME_OBJ_CLASS_NUM);
    return 0;
}

static int loadSmokingLabelName(const char *locationFilename, char *label[])
{
    printf("load smoking lable %s\n", locationFilename);
    readLines(locationFilename, label, SMOKING_OBJ_CLASS_NUM);
    return 0;
}

static int loadFaceLabelName(const char *locationFilename, char *label[])
{
    printf("load face lable %s\n", locationFilename);
    readLines(locationFilename, label, FACE_OBJ_CLASS_NUM);
    return 0;
}
...
int init_flame_post_process()
{
    int ret = 0;
    ret = loadFlameLabelName(FLAME_LABEL_NALE_TXT_PATH, labels);
    if (ret < 0)
    {
        printf("Load %s failed!\n", FLAME_LABEL_NALE_TXT_PATH);
        return -1;
    }
    return 0;
}

int init_smoking_post_process()
{
    int ret = 0;
    ret = loadSmokingLabelName(SMOKING_LABEL_NALE_TXT_PATH, labels);
    if (ret < 0)
    {
        printf("Load %s failed!\n", SMOKING_LABEL_NALE_TXT_PATH);
        return -1;
    }
    return 0;
}

int init_face_post_process()
{
    int ret = 0;
    ret = loadFaceLabelName(FACE_LABEL_NALE_TXT_PATH, labels);
    if (ret < 0)
    {
        printf("Load %s failed!\n", FACE_LABEL_NALE_TXT_PATH);
        return -1;
    }
    return 0;
}
...
const char *flame_cls_to_name(int cls_id)
{
    // 火焰检测只有一个类别：火焰
    if (cls_id == 0) {
        return "flame";
...
const char *smoking_cls_to_name(int cls_id)
{
    if (cls_id >= SMOKING_OBJ_CLASS_NUM)
    {
        return "null";
    }
...
const char *face_cls_to_name(int cls_id)
{
    if (cls_id >= FACE_OBJ_CLASS_NUM)
    {
        return "null";
    }
...
void deinit_flame_post_process()
{
    for (int i = 0; i < FLAME_OBJ_CLASS_NUM; i++)
    {
        if (labels[i] != nullptr)
        {
            free(labels[i]);
...
void deinit_smoking_post_process()
{
    for (int i = 0; i < SMOKING_OBJ_CLASS_NUM; i++)
    {
        if (labels[i] != nullptr)
        {
            free(labels[i]);
...
void deinit_face_post_process()
{
    for (int i = 0; i < FACE_OBJ_CLASS_NUM; i++)
    {
        if (labels[i] != nullptr)
        {
            free(labels[i]);
...
// 火焰检测专用后处理函数
int flame_post_process(rknn_app_context_t *app_ctx, void *outputs, letterbox_t *letter_box, float conf_threshold, float nms_threshold, flame_detect_result_list *od_results)
{
    // 使用与标准后处理相同的逻辑，但使用火焰检测的阈值
    return post_process(app_ctx, outputs, letter_box, FLAME_BOX_THRESH, FLAME_NMS_THRESH, (object_detect_result_list*)od_results);
}

// 吸烟检测专用后处理函数
int smoking_post_process(rknn_app_context_t *app_ctx, void *outputs, letterbox_t *letter_box, float conf_threshold, float nms_threshold, smoking_detect_result_list *od_results)
{
    // 使用与标准后处理相同的逻辑，但使用吸烟检测的阈值
    return post_process(app_ctx, outputs, letter_box, SMOKING_BOX_THRESH, SMOKING_NMS_THRESH, (object_detect_result_list*)od_results);
}

// 人脸检测专用后处理函数
int face_post_process(rknn_app_context_t *app_ctx, void *outputs, letterbox_t *letter_box, float conf_threshold, float nms_threshold, face_detect_result_list *od_results)
{
    // 使用与标准后处理相同的逻辑，但使用人脸检测的阈值
    return post_process(app_ctx, outputs, letter_box, FACE_BOX_THRESH, FACE_NMS_THRESH, (object_detect_result_list*)od_results);
}
