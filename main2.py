...
def reconnect_camera():
    ...
    # 新增：设置自动曝光模式
    ret = cam.MV_CC_SetEnumValue("ExposureMode", MV_EXPOSURE_AUTO_MODE_CONTINUOUS)  # 自动曝光模式
    if ret != 0:
...
def main():
    ...
    try:
        ...
        # 新增：初始化视频保存队列（最大100帧防内存溢出）
        save_queue = queue.Queue(maxsize=100)
        # 新增：视频写入器（在保存线程中初始化）
        video_writer = None
        # 新增：保存线程退出标志
        save_thread_exit = False

        # 定义保存线程函数
        def save_thread_func():
            nonlocal video_writer, save_thread_exit
            while not save_thread_exit:
                try:
                    # 从队列获取帧（超时1秒防阻塞）
                    ...
                    if video_writer is None:
                        ...
                        save_fps = max(1, int(round(current_fps)))
                        h, w = frame.shape[:2]
                        ...
                        print(f"视频保存启动：分辨率{w}x{h}, 保存帧率{save_fps}fps")
        ...
        # 启动保存线程
        save_thread = threading.Thread(target=save_thread_func, daemon=True)
        save_thread.start()
        ...
        while not g_bExit:
            ...
            # 新增：将处理后的帧放入保存队列（非阻塞，满则丢弃旧帧）
            try:
                save_queue.put(result_frame, block=False)
            except queue.Full:
                # 队列满时丢弃最旧帧（可选：打印提示）
                save_queue.get()
                save_queue.put(result_frame, block=False)
        ...
        # 清理资源
        g_bExit = True
        save_thread_exit = True  # 通知保存线程退出
        ...
        if video_writer is not None and video_writer.isOpened():
            ...
            print("视频已保存至 processed_output.mp4")
