    ...
    try:
        if udp_server:
          if task_code == 3:  # 红外入侵检测
            ...
            data_smoke = (1, 1, 0, 5,1, state, 0, 0, 0)
...
def udp_receive_commands(task_manager):
    ...
    global class_flame
    global class_inf
    global class_hardhat  # 添加安全帽检测全局变量
    try:
        while True:
            ...
            # 打印收到的任务及操作
                ...
                if frame_data[3] == 5:  #需要吸烟检测数据
                    # 这里需要从吸烟检测任务中获取状态
                    # 由于吸烟检测是独立任务，我们需要一个全局变量来存储状态
                    global class_smoke
                    if 'class_smoke' not in globals() or class_smoke is None:
